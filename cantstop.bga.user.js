...
const CS_DEFAULT_WIN_CHIPS_COUNT = 3;
...
var cantStopBgaUserscriptData = {
    ...
    init: function () {
        ...
        if (this.game.required_column_count != null) {
            this.winChipsCount = parseInt(this.game.required_column_count);
        }
        ...
        return this.possibleOutcomesValues
            .filter(sums =>
                    ...
                    && (
                        ...
                        || args.length < this.blackChipsCount
                    )
            ).length;
    },
    ...
    showLogDesktopMessage: function (message) {
        ...
        const logElement =
            ...
            `    <div class="roundedbox" style="background-color: lightgray;">CS: ${message}<div class="msgtime">${time}</div></div>` +
            `</div>`;
    ...
    showLogMobileMessage: function (message) {
        ...
        const logElement =
            ...
            `    <div class="roundedboxinner">CS: ${message}<div class="msgtime">${time}</div></div>` +
            `</div>`;
